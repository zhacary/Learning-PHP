        ...
        ?>
          ...
          <form method="POST">
              ...
              if ($_SERVER["REQUEST_METHOD"]=="POST") {
                    ...
                    if ($kode=='' || $nama=='' || $nilai=='' || $ID_User=='' || $kmapel=='' || $kjurusan=='') {
                  
                    }else{
                        $simpan=mysqli_query($koneksi,"INSERT INTO jawaban_pg (`kode_pg`,`nama_siswa`,`nilai_pg`,`ID_User`,`kode_mapel`,`kode_jurusan`) VALUES ('$kode','$nama','$nilai','$ID_User','$kmapel','$kjurusan');");

                        if ($simpan) {
                            echo "<script>window.location.href='ujian_pg_mhs.php'</script>";
                        }
                    }
              ...
              <input type="text" name="kode_pg" value="<?= $pg['kode_pg'] ?>"  class='hidden' readonly/>
               ...
               <?php $pilih=mysqli_query($koneksi,"SELECT * FROM ujian_pg WHERE kode_pg='$_GET[id_pg]'");
               if ($pg=mysqli_fetch_array($pilih)){?>
              <input type="text" name="kode_mapel" value="<?= $pg['kode_mapel'] ?>"  class='hidden' readonly/>
               <?php } ?>

               <?php $pilih=mysqli_query($koneksi,"SELECT * FROM ujian_pg WHERE kode_pg='$_GET[id_pg]'");
               if ($pg=mysqli_fetch_array($pilih)){?>
              <input type="text" name="kode_jurusan" value="<?= $pg['kode_jurusan'] ?>"  class='hidden' readonly/>
               <?php } ?>
